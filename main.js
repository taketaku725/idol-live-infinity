...
document.getElementById("rest").onclick = () => {
  ...
  popularity -= lostPop; money -= lostMoney;
...
function buyUpgrade(type, baseCost, level) {
  ...
  switch (type) {
    ...
    case "luck": luckLevel++; if (luckLevel > MAX_LUCK) luckLevel = MAX_LUCK;
      setMessage(`運Lv${luckLevel}🍀 上昇！`); updateLuckBar(); break;
    case "stamina": staminaLevel++; const gain = 10 + staminaLevel * 2;
      maxStamina += gain; setMessage(`体力Lv${staminaLevel}💪 最大体力 +${gain}`); break;
  }
  update();
}
...
function updatePrices() {
  ...
  list.forEach(([key,base,lv])=>{
    const item=shopItems[key]; if(!item||item.style.display==="none")return;
    ...
    btn.onclick=()=>{
      if(cantBuy){if(maxed)setMessage("🍀 これ以上は天運の極み！");else setMessage("お金が足りないよ…！");return;}
...
function checkUnlocks(){
  if(popularity>=1000&&shopItems.stream.style.display==="none"){
    shopItems.stream.style.display="flex";shopItems.luck.style.display="flex";
    setMessage("🎥 配信活動・運アップが解放された！");
  }
  if(popularity>=3000&&shopItems.goods.style.display==="none"){
    shopItems.goods.style.display="flex";setMessage("🛍 グッズ販売が解放された！");
  }
  if(popularity>=10000&&shopItems.fanclub.style.display==="none"){
    shopItems.fanclub.style.display="flex";setMessage("💌 ファンクラブが解放された！");
  }
  if(popularity>=30000&&shopItems.ad.style.display==="none"){
    shopItems.ad.style.display="flex";setMessage("🪧 広告契約が解放された！");
  }
  if(popularity>=100000&&shopItems.sponsor.style.display==="none"){
    shopItems.sponsor.style.display="flex";setMessage("🤝 スポンサー契約が解放された！");
  }
...
function tryRandomEvent(){
  ...
  if(Math.random()<baseChance){
    ...
    if(rewardType<0.5){
      ...
      money+=gain;eventMessage.textContent=`💸 ラッキー！臨時収入 +${formatNumber(gain)}円✨`;
    }else{
      const gain=Math.floor(30+Math.random()*70);
      popularity+=gain;eventMessage.textContent=`🎉 話題沸騰！人気 +${formatNumber(gain)}✨`;
    }
...
function doRebirth(){
  ...
  inflationTotal+=add;lastRebirthBonus=add;rebirthCount++;
